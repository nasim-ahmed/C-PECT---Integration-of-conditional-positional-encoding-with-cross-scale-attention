...
#Stolen from https://arxiv.org/pdf/2112.13492.pdf
class SPT(nn.Module):
    def __init__(self, *, img_size, dim, patch_size, channels = 3):
        super().__init__()
        img_size = to_2tuple(img_size)
        self.patch_size = patch_size

        patch_dim = patch_size * patch_size * 5 * channels
        self.H, self.W = img_size[0] // patch_size, img_size[1] // patch_size
    ...
    def forward(self, x):
        ...
        return self.to_patch_tokens(x_with_shifts), (H,W)
...
class GLobalCrossAttention(nn.Module):
    # def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
    #     pass
    def __init__(self, dim, num_heads, group_size=7, interval=8, mlp_ratio=4., qkv_bias=True,
...
class GroupBlock(TimmBlock):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, ws=1, interval=8):
        ...
        if ws == 1:
            ...
            self.attn = GLobalCrossAttention(dim=dim,
                             num_heads=num_heads, group_size=7, interval=interval,
                             mlp_ratio=mlp_ratio,
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(len(depths)):
            if i == 0:
                ...
                spt_patch_embedding = SPT(img_size=img_size, dim=embed_dims[i], patch_size=patch_size, channels=in_chans)
                self.patch_embeds.append(spt_patch_embedding)
            else:
                ...
                self.patch_embeds.append(
                    SPT(img_size=img_size, dim=embed_dims[i],patch_size= 2, channels=embed_dims[i - 1]))
...
class ALTGVT(PCPVT):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 ...
                 F4=False, extra_norm=False, crs_interval=[8, 4, 2, 1]):
        ...
        for k in range(len(depths)):
            _block = nn.ModuleList([block_cls(
                dim=embed_dims[k], num_heads=num_heads[k], mlp_ratio=mlp_ratios[k], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
                sr_ratio=sr_ratios[k], ws=1 if i % 2 == 1 else wss[k], interval=crs_interval[k]) for i in range(depths[k])])
            self.blocks.append(_block)
            cur += depths[k]
        self.apply(self._init_weights)
...
class tft_gvt_small(ALTGVT):
    def __init__(self, **kwargs):
        super(tft_gvt_small, self).__init__(
